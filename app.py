    ...
    if "." not in page:
        ...
        if os.path.exists(candidate):
            return send_from_directory(
                os.path.join(app.static_folder, "pages"),
                page + ".html"
            )
...
def api_admin_login():
    ...
    try:
        ...
        if not a or not check_password_hash(
            a["password_hash"],
            password
        ):
...
def api_slots():
    ...
    try:
        cur.callproc("AutoCompleteBookings")
        conn.commit()
...
def api_create_booking():
    ...
    if not session.get("user_id"):
        return jsonify({
            "error": "Log in with your player account to book "
                     "(admin session cannot place bookings)."
        }), 403
    ...
    try:
        ...
        if rows:
            ...
            cur.execute(
                ...
                FROM booking_slots
                WHERE booking_id IN ({fmt})
                """,
                ids
            )

            for s in cur.fetchall():
                slots_map.setdefault(
                    s["booking_id"],
                    []
                ).append(s["slot_id"])
...
def api_get_booking(bid):
    ...
    try:
        ...
        return jsonify({
            "booking": _row_to_booking(b, slots)
        })
